  ...
  <q-page class="flex flex-top">
        ...
        <template v-slot:body-cell="props">
          <q-td :props="props">
            ...
            <div v-else-if="props.col.name === 'action'">
              ...
              <q-btn round flat color="green" icon="check_circle" @click="confirmDN(props.rowIndex)"
                v-if="props.row.status !== 'confirmed' && props.row.status !== 'shipped'" />
    ...
    <q-dialog v-model="formData">
      <q-card style="width: 500px; max-width: 90vw" class="q-px-sm q-pb-md">
        <q-card-section><div class="text-h6">{{ mode === 'create' ? t('new') : t('edit') }} - DN</div></q-card-section>
        <q-card-section class="q-pt-none">
          <q-input dense v-model="form.dn_code" :label="t('wms.dnCode')" />
          <q-input dense v-model="form.dn_type" :label="t('wms.dnType')" />
          <q-input dense v-model="form.customer_name" :label="t('wms.customerName')" />
        </q-card-section>
...
const cols = computed(() => [
  { name: 'dn_code', required: true, label: t('wms.dnCode'), align: 'left', field: 'dn_code' },
  { name: 'dn_type', label: t('wms.dnType'), field: 'dn_type' },
  { name: 'customer_name', label: t('wms.customerName'), field: 'customer_name' },
...
function statusColor(s) { return s === 'confirmed' || s === 'shipped' ? 'green' : 'orange' }

function onRequest(p) {
  let rd = p || { pagination: pagination.value }
  const params = search.value ? JSON.stringify({ data__dn_code__icontains: search.value }) : '{}'
  get({ url: 'core/dn/', params: { params, page: rd.pagination.page, max_page: rd.pagination.rowsPerPage } })
    .then(res => {
...
function deleteData(e) {
  $q.dialog({ dark: $q.dark.isActive, title: t('delete'), message: t('confirmnotice'), cancel: true })
    .onOk(() => post('core/dn/delete/', { id: rows.value[e].id }).then(() => onRequest())
      .catch(err => { $q.notify({ type: 'error', message: err?.detail || 'Error' }); $q.loading.hide() }))
}

function confirmDN(e) {
  $q.dialog({ dark: $q.dark.isActive, title: t('wms.confirmDN'), message: t('confirmnotice'), cancel: true })
    .onOk(() => post('wmsapp/dn/confirm/', { id: rows.value[e].id }).then(() => onRequest())
      .catch(err => { $q.notify({ type: 'error', message: err?.detail || 'Error' }); $q.loading.hide() }))
}
...
async function submitData() {
  ...
  const url = mode.value === 'create' ? 'core/dn/create/' : 'core/dn/update/'
